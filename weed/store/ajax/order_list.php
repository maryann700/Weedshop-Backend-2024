...
if(isset($_POST['type'])){
	...
	/*for($i=0;$i<count($orders);$i++) {
		$html .= '<tr>';
		$html .= '<td><a href=javascript:;>'.$orders[$i]['name'].'</a></td>';
		$html .= '<td>'.date('d F, Y',strtotime($orders[$i]['order_date'])).'</td>';
		$html .= '<td>$'. $orders[$i]['final_total'].'</td>';
		$html .= '<td><span class="label label-sm label-"'. $orders[$i]['statusclass'].'>'.$orders[$i]['status'].' </span></td>';
		$html .= '<td><a target=_blank href=order_view.php?id='.$orders[$i]['orderid'].' class=btn btn-sm btn-default>
